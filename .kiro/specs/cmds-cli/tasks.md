...
- [x] 1. 定义核心类型与数据模型
  - [x] 1.1 在 `src/types.ts` 中定义所有共享类型（TldrEntry, TldrIndex, CommandEntry, RuntimeIndex, RuntimeIndexMeta, SearchResult, CommandInfo, CategorySummary, ListSummary, RouteResult, ScanResult）
    - 严格按照设计文档中的数据模型定义
    - _Requirements: 7.4_

- [x] 2. 实现数据层与输出格式化
  - [x] 2.1 实现 `src/data.ts` — 索引加载与保存
  ...
  - [x] 2.3 实现 `src/formatter.ts` — 输出格式化
...
- [x] 3. 实现工具函数与智能路由
  - [x] 3.1 实现 `src/utils.ts` — 工具函数
    - 实现 commandExists（PATH 查找）、execCommand（执行外部命令）等辅助函数
    - _Requirements: 3.4, 5.1_

  - [x] 3.2 实现 `src/router.ts` — 智能路由
...
- [x] 6. 实现 Info Resolver
  - [x] 6.1 实现 `src/info.ts`
...
- [x] 7. 实现 List Aggregator
  - [x] 7.1 实现 `src/list.ts`
...
- [x] 10. 集成 CLI 入口与子命令注册
  - [x] 10.1 更新 `src/index.ts` — 注册所有子命令
...
- [x] 11. 实现 fast-check 自定义 Arbitrary 生成器
  - [x] 11.1 创建 `src/__tests__/helpers/arbitraries.ts`
